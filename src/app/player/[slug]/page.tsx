...
export default function PlayerPage({params}: { params: { slug: string } }) {
            ...
            <div className={style.statSectionDiv}>
                ...
                <div className={style.tableDiv}>
                    <table className={style.table}>
                        ...
                        <tbody>
                        {playerStats?.batting?.map((stat: BattingStats) => (
                            <tr
                            ...
                            >
                                ...
                                <td className={style.cell}>{stat.G}</td>
                                <td className={style.cell}>{stat.AB}</td>
                                <td className={style.cell}>{stat.R}</td>
                                <td className={style.cell}>{stat.H}</td>
                                <td className={style.cell}>{stat['2B']}</td>
                                <td className={style.cell}>{stat['3B']}</td>
                                <td className={style.cell}>{stat.HR}</td>
                                <td className={style.cell}>{stat.RBI}</td>
                                <td className={style.cell}>{stat.SB}</td>
                                <td className={style.cell}>{stat.CS}</td>
                                <td className={style.cell}>{stat.BB}</td>
                                <td className={style.cell}>{stat.SO}</td>
                                <td className={style.cell}>{stat.IBB}</td>
                                <td className={style.cell}>{stat.HBP}</td>
                                <td className={style.cell}>{stat.SH}</td>
                                <td className={style.cell}>{stat.SF}</td>
                                <td className={style.cell}>{stat.GIDP}</td>
                            </tr>
                        ))}
                        </tbody>
                    </table>
                        ...
                        {playerStats?.pitching?.map(
                            (stat: PitchingStats) => (
                                ...
                                >
                                    ...
                                    <td className={style.cell}>{stat.W}</td>
                                    <td className={style.cell}>{stat.L}</td>
                                    <td className={style.cell}>{stat.G}</td>
                                    <td className={style.cell}>
                                        {stat.GS}
                                    </td>
                        ...
                        {playerStats?.fielding?.map(
                            (stat: FieldingStats) => (
                                ...
                                >
                                    ...
                                    <td className={style.cell}>{stat.A}</td>
                                    <td className={style.cell}>{stat.E}</td>
                                    <td className={style.cell}>
                                        {stat.DP}
                                    </td>
                                </tr>
    ...
    function CareerStatsTable() {
        ...
        if (playerStats.batting && playerStats.batting.length > 0) {
            const totalG = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.G || 0),
                0
            )
            const totalAB = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.AB || 0),
                0
            )
            const totalR = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.R || 0),
                0
            )
            const totalH = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.H || 0),
                0
            )
            const total2B = playerStats.batting.reduce(
                (acc, stat) => acc + (stat['2B'] || 0),
                0
            )
            const total3B = playerStats.batting.reduce(
                (acc, stat) => acc + (stat['3B'] || 0),
                0
            )
            const totalHR = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.HR || 0),
                0
            )
            const totalRBI = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.RBI || 0),
                0
            )
            const totalSB = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.SB || 0),
                0
            )
            const totalCS = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.CS || 0),
                0
            )
            const totalBB = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.BB || 0),
                0
            )
            const totalSO = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.SO || 0),
                0
            )
            const totalIBB = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.IBB || 0),
                0
            )
            const totalHBP = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.HBP || 0),
                0
            )
            const totalSF = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.SF || 0),
                0
            )
            const totalGIDP = playerStats.batting.reduce(
                (acc, stat) => acc + (stat.GIDP || 0),
                0
            )
        ...
        if (playerStats.pitching && playerStats.pitching.length > 0) {
            const totalW = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.W || 0),
                0
            )
            const totalL = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.L || 0),
                0
            )
            const totalG = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.G || 0),
                0
            )
            const totalGS = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.GS || 0),
                0
            )
            const totalCG = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.CG || 0),
                0
            )
            const totalSHO = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.SHO || 0),
                0
            )
            const totalSV = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.SV || 0),
                0
            )
            const totalIPouts = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.IPouts || 0),
                0
            )
            const totalH = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.H || 0),
                0
            )
            const totalER = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.ER || 0),
                0
            )
            const totalHR = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.HR || 0),
                0
            )
            const totalBB = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.BB || 0),
                0
            )
            const totalSO = playerStats.pitching.reduce(
                (acc, stat) => acc + (stat.SO || 0),
                0
            )
        ...
        if (playerStats.fielding && playerStats.fielding.length > 0) {
            const totalG = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.G || 0),
                0
            )
            const totalGS = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.GS || 0),
                0
            )
            const totalInnOuts = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.InnOuts || 0),
                0
            )
            const totalPO = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.PO || 0),
                0
            )
            const totalA = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.A || 0),
                0
            )
            const totalE = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.E || 0),
                0
            )
            const totalDP = playerStats.fielding.reduce(
                (acc, stat) => acc + (stat.DP || 0),
                0
            )
                ...
                {playerStats.batting && playerStats.batting.length > 0 && (
                    <div className={style.tableDiv}>
                        <table className={style.table}>
                            <thead className={style.tableHeader}>
                            <tr className={style.tableRow}>
                                <th className={style.tableClickHeader}></th>
                                <th className={style.cell}>G</th>
                                <th className={style.cell}>AB</th>
                                <th className={style.cell}>R</th>
                                <th className={style.cell}>H</th>
                            ...
                            <tbody>
                            <tr className={style.tableRow}>
                                <td className={style.cell}>Batting</td>
                                <td className={style.cell}>
                                    {totalBattingStats.G}
                                </td>
                                <td className={style.cell}>
                ...
                {playerStats.pitching && playerStats.pitching.length > 0 && (
                    <div className={style.tableDiv}>
                        <table className={style.table}>
                            ...
                            <tbody>
                            <tr className={style.tableRow}>
                                <td className={style.cell}>Pitching</td>
                                <td className={style.cell}>
                ...
                {playerStats.fielding && playerStats.fielding.length > 0 && (
                    <div className={style.tableDiv}>
                        <table className={style.tableRow}>
                            ...
                            <tbody>
                            <tr className={style.tableRow}>
                                <td className={style.cell}>Fielding</td>
                                <td className={style.cell}>
    ...
    return (
        <div className={style.pageCard}>
            <div className={style.innerCard}>
                <div className={style.statSectionDiv}>
                    ...
                    <div>
                        ...
                        {playerData.deathYear ? (
                            <p>
                                Died: {playerData.deathMonth}/
                                {playerData.deathDay}/{playerData.deathYear}
                            </p>
                        ) : (
                            <p></p>
                        )}
                    ...
                    <div>
                        ...
                        <p>
                            Height:{' '}
                            {(Number(playerData.height) / 12).toFixed(0)} feet,{' '}
                            {Number(playerData.height) % 12} inches
                        </p>
                    </div>
                    <div>
                ...
                <h2 className={style.h2 + ' text-center'}>Career Statistics</h2>
                {<CareerStatsTable/>}

                <h2 className={style.h2 + ' text-center'}>Season Statistics</h2>
