...
function setupContext(
    searchDisplay: {
        ...
        message?: string
        dbResult?: {
            rows: { playerID: string; teamID: string; yearID: string }[]
        }
...
test('renders search results with error message', async () => {
    ...
    expect(
        screen.getByText('An error occurred while fetching the search results.')
    ).toBeInTheDocument()
...
test('renders search results with dbResult', async () => {
    const searchDisplay = {
        dbResult: {rows: [{playerID: '1', teamID: '2', yearID: '2020'}]},
    }
    const contextValue = setupContext(searchDisplay, null, '')
...
test('renders no search results found', async () => {
    ...
    expect(
        screen.getByText('Please try a different search query.')
    ).toBeInTheDocument()
})
